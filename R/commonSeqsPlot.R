...
commonSeqsPlot <- function(sample1, sample2, productive_aa, show = "common") {
    ...
    if (show == "common") {
        common <- commonSeqs(samples = c(sample1, sample2), productive_aa = productive_aa)
        plot <- ggplot(data = common, aes_string(x = names(common)[2], y = names(common)[3], label = "aminoAcid")) + 
            geom_point() +
            theme_minimal() + 
            labs(x = paste(sample1, "frequency (%)"), y = paste(sample2, "frequency (%)"))
    }
    if (show == "all") {
        stable1 <- productive_aa %>% filter(sample == sample1)
        stable2 <- productive_aa %>% filter(sample == sample2)
        all <- full_join(stable1, stable2, by = "aminoAcid") %>% mutate_all(~replace(., is.na(.), 0))
        all <- all %>% recode(frequencyCount.x = sample1, frequencyCount.y = sample2)
