...
#' @import tidyverse
commonSeqsVenn <- function(samples, productive_aa) {
    if (length(samples) > 3 | length(samples) < 2) {
        stop("Please enter 2 or 3 samples.")
    }
    if (length(samples) == 2) {
        a <- productive_aa %>% filter(sample == samples[[1]])
        b <- productive_aa %>% filter(sample == samples[[2]])
        grid::grid.newpage()
    ...
    if (length(samples) == 3) {
        a <- productive_aa %>% filter(sample == samples[[1]])
        b <- productive_aa %>% filter(sample == samples[[2]])
        c <- productive_aa %>% filter(sample == samples[[3]])
        grid::grid.newpage()
