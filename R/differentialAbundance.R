...
#' @param q A numeric value between 0.0 and 1.0 indicating the threshold Holms
...
#'  will not be shown.
#' @param zero A numeric value to set all zero values to when calculating the
...
#' @return Returns a data frame with columns corresponding to the frequency of
...
differentialAbundance <- function(
    study_table, repertoire_ids = NULL,
  ...
  fisher_table <- study_table |>
    ...
    dplyr::mutate(
      not_x = base::sum(!!base::as.name(repertoire_ids[1])) -
        !!base::as.name(repertoire_ids[1]),
      not_y = base::sum(!!base::as.name(repertoire_ids[2])) -
