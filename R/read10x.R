...
read10x <- function(path, recursive = FALSE) {
    ...
    progress_bar <- progress::progress_bar$new(format = "Reading 10x files [:bar] :percent eta: :eta",
                                           total = length(file_paths), clear = FALSE, width = 60)
    progress_bar$tick(0)
    file_list <- file_paths %>% 
                 purrr::map(~ collpase_chains(.x, progress_bar)) %>%
                 dplyr::bind_rows()
...
standardize10x <- function(clone_data) {
