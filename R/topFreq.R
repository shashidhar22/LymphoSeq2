...
topFreq <- function(productive_table, percent = 0.1) {
    top_aa <- productive_table %>%  
              ...
              top_frac(percent, wt=frequencyCount) %>% 
              as_vector()
    top_freq <- productive_table %>% 
                filter(aminoAcid %in% top_aa) %>%
                group_by(aminoAcid) %>%
                summarise(minFrequency = min(frequencyCount),
                          maxFrequency = max(frequencyCount),
                          meanFrequency = mean(frequencyCount),
                          numberSamples = length(frequencyCount > 0)) %>%
                arrange(desc(numberSamples), desc(meanFrequency))
    

    top_freq <- left_join(top_freq, LymphoSeqDB::prevalenceTRB, by="aminoAcid") %>% mutate(prevalence = replace_na(0))
    antigen_table <- LymphoSeqDB::publishedTRB %>% as_tibble() %>% select(aminoAcid, antigen)
    top_freq <- left_join(top_freq, antigen_table, by="aminoAcid")  
    return(top_freq)
}