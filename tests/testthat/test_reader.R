...
test_that("Reads a list of AIRR file correctly", {
  ...
  stable <- stable |>
    ...
    dplyr::arrange(repertoire_id)
  expect_equal(srow, 2000)
  expect_true(isTRUE(all.equal(ctable |> dplyr::arrange(repertoire_id), stable, check.attributes = FALSE)))
})
...
test_that("Reads AIRR files from a path correctly", {
  ...
  srow <- base::nrow(stable)
  ctable <- tibble::tibble(
    repertoire_id = c("015V12001549_CFAR", "015V12001685_CFAR_R", "015V12003105_CFAR", "015V06013979_CFAR"),
    tot_freq = c(1, 1, 1, 1),
