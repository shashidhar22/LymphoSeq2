...
# Automatically detects AIRR format or contig_annotations.csv
sc_data <- read10x("path/to/airr_rearrangement.tsv")
```

## Merging Paired Chains

```{r merge_chains, eval=FALSE}
# Strict: only cells with exactly 1 TRA + 1 TRB
paired <- merge_chains(sc_data, mode = "strict")

# Best: select most frequent chain when multiples exist
paired <- merge_chains(sc_data, mode = "best")
```

## Single-Cell Analysis

```{r sc_analysis, eval=FALSE}
# Works with all standard functions
diversity <- clonality(paired)
top_clones <- topSeqs(paired, top = 20)

# Chain-specific gene usage
alpha_v <- paired %>%
